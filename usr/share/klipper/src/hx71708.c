...
#define HX711S_SAMPLE_INIT_TICKS			(CONFIG_CLOCK_FREQ / 1000000 * 120)
...
enum {HX711S_SAMPLE_START = 1 << 0, HX711S_SAMPLE_INIT = 1 << 1};
...
command_config_hx711s(uint32_t *args)
{
	...
	h->init_ticks = HX711S_SAMPLE_INIT_TICKS;
...
get_hx711s(struct hx711s* h)
{
    ...
    if (is_data_valid == 0)
    {
		...
		for(uint8_t i = 0; i < 2; i++) //hx71708 27 pulse for setting 80Hz
		{
        	for (int j = 0; j < h->hx711_count; j++)
            	gpio_out_write(h->clks[j], 1);
        	for (int j = 0; j < h->hx711_count; j++)
...
command_query_hx711s(uint32_t *args)
{
	...
	struct hx711s* h = oid_lookup(oid,command_config_hx711s);	
	h->times_read = args[1];
	h->flags |= (HX711S_SAMPLE_START | HX711S_SAMPLE_INIT);
	sched_del_timer(&h->hx711s_timer);
	irq_disable();
	h->hx711s_timer.waketime = timer_read_time() + h->init_ticks;
...
hx711s_task(void)
{
	...
	foreach_oid(oid,h,command_config_hx711s)
	{
		...
		if((h->flags & HX711S_SAMPLE_INIT) == HX711S_SAMPLE_INIT) //init reset operation.
		{
			h->flags &= ~HX711S_SAMPLE_INIT;
